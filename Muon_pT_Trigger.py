# -*- coding: utf-8 -*-
"""
Code to filter out events where no muon has transverse momentum >1.78GeV.
...
thre = 1780 #MeV, threshold (value used in literature dependendt on LHCb version)
...
for i in range (len(loc_mu_plus)):
    if loc_mu_plus[i]==False and loc_mu_plus[i]==False:
...
print(f"Transverse muon momenta check (the first 100 events); diregarded events; threshold = {thre} MeV")
...
print(f"Transverse muon momenta check (the first 100 events); accepted events; threshold = {thre} MeV")
