...
def chebyshev(x, a_0, a_1, a_2):
    return a_0 + a_1 * x + a_2 * (2 * x ** 2 -1)
...
def norm_d2gamma_p_d2q2_dcosthetal(fl, afb, a_0, a_1, a_2, cos_theta_l):
    """
    Returns the pdf defined above
    :param fl: f_l observable
    :param afb: a_fb observable
    ...
    :return:
    """
    scalar_array = d2gamma_p_d2q2_dcosthetal(fl, afb, cos_theta_l) + chebyshev(cos_theta_l, a_0, a_1, a_2)
    cos_theta_l_acc = np.linspace(-1,1,10000)
    norm_factor = integrate(cos_theta_l_acc, d2gamma_p_d2q2_dcosthetal(fl=fl, afb=afb, cos_theta_l=cos_theta_l_acc) + chebyshev(cos_theta_l_acc, a_0, a_1, a_2))
    normalised_scalar_array = scalar_array /norm_factor  
    return normalised_scalar_array

def plotting_func(fl, afb, a_0, a_1, a_2, cos_theta_l):
    """
    Returns the pdf defined above
    :param fl: f_l observable
    :param afb: a_fb observable
    ...
    background = chebyshev(cos_theta_l, a_0, a_1, a_2)
    cos_theta_l_acc = np.linspace(-1,1,10000)
    norm_factor = integrate(cos_theta_l_acc, d2gamma_p_d2q2_dcosthetal(fl=fl, afb=afb, cos_theta_l=cos_theta_l_acc) + chebyshev(cos_theta_l_acc, a_0, a_1, a_2))
...
def log_likelihood_ctl(fl, afb, a_0, a_1, a_2, _bin):
    """
    ...
    _bin = bins[int(_bin)]
    ctl = _bin['costhetal']
    normalised_scalar_array = norm_d2gamma_p_d2q2_dcosthetal(fl=fl, afb=afb, a_0 = a_0, a_1 = a_1, a_2 = a_2, cos_theta_l= ctl)
...
for b in range(10):
    ...
    a_1s, a_1_errs = [], []
    a_2s, a_2_errs = [], []
    m = minuit(log_likelihood_ctl, fl=starting_point[0], afb=starting_point[1], a_0 = 0, a_1 = 0, a_2 = 0, _bin = b)
    ...
    """
    m.limits=((-1.0, 1.0), (-1.0, 1.0), (0,0), (0, 0), (0, 0), None) 
    m.migrad()
    m.hesse()
    bin_results_to_check = m
    fls.append(m.values[0])
    afbs.append(m.values[1])
    a_0s.append(m.values[2])
    a_1s.append(m.values[3])
    a_2s.append(m.values[4])
    fl_errs.append(m.errors[0])
    afb_errs.append(m.errors[1])
    a_0_errs.append(m.errors[2])
    a_1_errs.append(m.errors[3])
    a_2_errs.append(m.errors[4])
    ...
    norm_signal, norm_background= plotting_func(fls[0], afbs[0], a_0s[0], a_1s[0], a_2s[0], bins[b][i].sort_values())
    plt.errorbar((e[:-1] + 0.5*e[1] - 0.5*e[0]), h, yerr = np.sqrt(h * len(bins[b][i]))/len(bins[b][i]), fmt = '.', color = 'blue')
    plt.plot(bins[b][i].sort_values(), norm_d2gamma_p_d2q2_dcosthetal(fls[0], afbs[0],a_0s[0], a_1s[0], a_2s[0], bins[b][i].sort_values()), label = "Total fit")
