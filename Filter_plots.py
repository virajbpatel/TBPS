...
def histogram(data_frames, label, bin_range, bin_N):
    ...
    for i in range(len(data_frames)):
        ...
        print('Data set %s: %d/%d points left out for given bins' %
              (data_frames[i].name, left_out, len(x)))
    plt.legend()
...
def totnormalhistogram(data_frames, label, bin_range, bin_N):
    ...
    for i in range(len(data_frames)):
        ...
        err = np.sqrt(N)/len(x)
        b_vals = (b[1:] + b[:-1]) / 2
        plt.errorbar(b_vals, N_w, yerr=err, fmt='o', capsize=3,
                     markersize=2, label=data_frames[i].name)
        width = b[1]-b[0]
        # plt.bar(b_vals, N_w, width=width, alpha=0.2, linewidth=0)
        occur1 = x > bin_range[1]
        occur2 = x < bin_range[0]
        left_out = occur1.sum() + occur2.sum()
        print('Data set %s: %d/%d points left out for given bins, %f' %
              (data_frames[i].name, left_out, len(x), left_out/len(x)))
    plt.legend()
...
def normalhistogram(data_frames, label, bin_range, bin_N):
    ...
    for i in range(len(data_frames)):
        x = data_frames[i][str(label)].to_numpy()
        plt.hist(x, bins, label=data_frames[i].name,
...
def fittotnormalhistogram(data_frames, label, bin_range, bin_N):
    ...
    for i in range(len(data_frames)):
        ...
        params2, cov2 = curve_fit(gauss_backg, b_vals, N_w, p0=[
                                  0.1, 5280, 30, -4e-5, 0.2])
        print(params2)
        print(np.sqrt(abs(cov2)))
        line = params2[3]*np.array(b_vals) + params2[4]
        plt.plot(b_vals, line, 'k--')
        plt.plot(b_vals, gauss_backg(
            b_vals, params2[0], params2[1], params2[2], params2[3], params2[4]), 'r--')
...
def fittotnormalhistogram_exp(data_frame, label, bin_range, bin_N):
    ...
    b_vals = (b[1:] + b[:-1]) / 2
    width = b[1]-b[0]
    plt.errorbar(b_vals, N_w, xerr=width/2, yerr=err, fmt='ko',
                 capsize=0, markersize=3, label=data_frame.name)

    # plt.bar(b_vals, N_w, width=width, alpha=0.2, linewidth=0)

    params, cov = curve_fit(gauss_exp, b_vals, N_w, p0=[
                            0.18, 5280, 20, 0.6, 0.02, 5010, 0.002])
    print(params)
    ...
    plt.plot(b_vals, gauss_exp(
        b_vals, params[0], params[1], params[2], params[3], params[4], params[5], params[6]), 'r--')

    occur1 = x > bin_range[1]
    occur2 = x < bin_range[0]
    left_out = occur1.sum() + occur2.sum()
    print('Data set %s: %d/%d points left out for given bins, %f' %
          (data_frame.name, left_out, len(x), left_out/len(x)))
    plt.legend()
    title = str(label) + ' Nomralised Histogram'
    plt.title(title)
    plt.xlabel(r'Reconstructed $B^0$ mass (MeV/c$^2$)')
...
def cosl_histogram(data_frame, label, bin_range, bin_N, bin_name):
    ...
    for i in range(len(data_frame)):
        q2bin = data_frame[i][data_frame[i].q2.between(
        ...
        b_vals = (b[1:] + b[:-1]) / 2
        width = b[1]-b[0]
        plt.errorbar(b_vals, N, xerr=width/2, yerr=err, fmt='ko',
                     capsize=0, markersize=5, label=data_frame[i].name)
    plt.legend()
    title = str(bin_name) + ' ' + str(q2bin.name) + \
        r' $\cos (\theta_l)$ Normalised Histogram'
    plt.title(title)
...
def cosl_histogram_normal(data_frame, label, bin_range, bin_N, bin_name):
    ...
    for i in range(len(data_frame)):
        ...
        b_vals = (b[1:] + b[:-1]) / 2
        width = b[1]-b[0]
        plt.errorbar(b_vals, N_w, xerr=width/2, yerr=err, fmt='o',
                     capsize=0, markersize=5, label=data_frame[i].name)
    plt.legend()
    title = str(bin_name) + ' ' + str(q2bin.name) + \
        r' $\cos (\theta_l)$ Normalised Histogram'
