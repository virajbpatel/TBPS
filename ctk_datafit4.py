...
import MODULE_curve_fitting_functions_with_legendre as mcf
...
def pdf(fl, _bin, cos_theta_k): #!!
    ctk = cos_theta_k
    s2tk = 1 - ctk**2 # sine-squared term
    scalar_array = (3/2) * (fl * ctk**2) + (3  /4) * (1 - fl) * s2tk    
    return scalar_array * lg.legval(ctk, X_PARAMS[_bin])

def norm_pdf_without_bg(fl, _bin, cos_theta_k): #!!
    cos_theta_k_acc = np.linspace(-1,1,10000) #!!
    norm_factor = integrate2(cos_theta_k_acc, pdf(fl=fl, _bin = _bin, cos_theta_k=cos_theta_k_acc))
    return pdf(fl, _bin, cos_theta_k)/norm_factor

def pdf_with_bg(fl, a0, a1, a2, _bin, cos_theta_k): #!!!
    R = BG_RATIO[_bin]/(1-BG_RATIO[_bin]) # BG_RATIO is defined in the next cell
    norm_scalar_array = norm_pdf_without_bg(fl, _bin, cos_theta_k) 
    return norm_scalar_array +  R*norm_chebyshev(cos_theta_k, a0, a1, a2)

def norm_pdf(fl, a0, a1, a2, _bin, cos_theta_k): #!! 
    scalar_array = pdf_with_bg(fl, a0, a1, a2, _bin, cos_theta_k)
    cos_theta_k_acc = np.linspace(-1,1,10000)
    norm_factor = integrate2(cos_theta_k_acc, pdf_with_bg(fl, a0, a1, a2, _bin, cos_theta_k_acc))
    normalised_scalar_array = scalar_array /norm_factor  
    return normalised_scalar_array

def log_likelihood_ctl(fl, a0, a1, a2, _bin): #!!
    ...
    ctk = _BIN['costhetak']
    normalised_scalar_array = norm_pdf(fl=fl, a0=a0, a1=a1, a2=a2, _bin = int(_bin), cos_theta_k= ctk)
    return - np.sum(np.log(normalised_scalar_array))

def norm_factor_signal_bg(fl, a0, a1, a2, _bin): #!!
    cos_theta_k = np.linspace(-1,1,10000) #!!
    Y = pdf_with_bg(fl, a0, a1, a2, _bin, cos_theta_k)
    norm_factor = integrate2(cos_theta_k, Y)
    return norm_factor

def chi_sq(x, data, fls, a0, a1, a2, b):
    X = 0
    N = sum(data)
    dx = x[1] - x[0]
    for i in range(len(data)):
        X += ((N*norm_pdf(fls, a0, a1, a2, b, cos_theta_k = x[i])*dx - data[i]) ** 2) / data[i]
    return X/(len(data) + 5)
#%%
"""
...
for i in range(B):
    ...
    n, e = np.histogram(BINS_MC[i][v], bins = NO_OF_BINS) 
    plt.hist(BINS_MC[i][v], bins = NO_OF_BINS)
...
for b in range(10):            
    ...
    decimal_places = 3
    ST = [SM_FL[b], CHEBY_PARAMS[b][0], CHEBY_PARAMS[b][1], CHEBY_PARAMS[b][2]] #!!
    ...
    m = minuit(log_likelihood_ctl, fl=ST[0], a0=ST[1], a1=ST[2], a2=ST[3], _bin = b)
    ...
    """
    m.limits=((-1.0, 1.0), #!!
              (ST[1]-SD[1], ST[1]+SD[1]), (ST[2], ST[2]), (ST[3], ST[3]), None) 
    m.migrad()
    m.hesse()
    bin_results_to_check = m
    fls.append(m.values[0]) #!!
    # afbs.append(m.values[1]) #!!
    a0s.append(m.values[1])
    a1s.append(m.values[2])
    a2s.append(m.values[3])
    fl_errs.append(m.errors[0]) #!!
    # afb_errs.append(m.errors[1]) #!!
    a0_errs.append(m.errors[1])
    a1_errs.append(m.errors[2])
    a2_errs.append(m.errors[3])
    ...
    signal = norm_pdf_without_bg(fls[0],  b, X)                                                                        
    background = BG_RATIO[b]/(1-BG_RATIO[b]) * norm_chebyshev(X, a0s[0], a1s[0], a2s[0])
    norm_factor = norm_factor_signal_bg(fls[0], a0s[0], a1s[0], a2s[0], b)
    ...
    plt.plot(X, norm_pdf(fls[0], a0s[0], a1s[0], a2s[0], b, BINS[b][v].sort_values()),
             label = f"Total Fit: fl = {np.round(fls, decimal_places)} pm {np.round(fl_errs, decimal_places)}")
    ...
    plt.plot(0, '.', label = f'SM vals: fl = {SM_FL[b]}', color = 'grey') 
